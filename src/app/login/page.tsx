...
export default function SignInPage() {
  ...
  const handleGoogleResponse = async (response: any) => {
    try {
      ...
      toast({
        ...
        description: "Selamat datang kembali di MarketPoint!",
      });
      router.push("/");
    } catch (error: any) {
      toast({
        ...
        description: error.message || "Pastikan domain Anda sudah terdaftar di Google Cloud Console.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const triggerGoogleSignIn = () => {
    if (window.google?.accounts?.id) {
      // Mencoba memicu prompt jika tombol tersembunyi gagal
      window.google.accounts.id.prompt();
      
      // Fallback: mencoba klik pada tombol asli yang dirender Google
