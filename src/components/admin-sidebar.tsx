        ...
        <div className="flex items-center gap-2.5">
          ...
          {state !== "collapsed" && (
            <div className="flex flex-col overflow-hidden">
              <span className="text-[10px] font-medium text-red-600 leading-none">Admin Control</span>
                    ...
                    <SidebarMenuItem key={item.label}>
                      <SidebarMenuButton asChild isActive={isActive} tooltip={item.label}>
                        <Link href={item.href} className={cn(
                          ...
                          isActive ? "text-red-600 bg-red-50/50" : "text-muted-foreground hover:text-foreground"
                        )}>
        ...
        {state !== "collapsed" && user && (
          <div className="mt-3 p-2 rounded-lg bg-muted/30 border border-border flex items-center gap-2">
            <div className="h-7 w-7 rounded-md bg-red-100 flex items-center justify-center shrink-0">
               <span className="text-[10px] font-bold text-red-600">{user.displayName?.substring(0, 1) || "A"}</span>
            </div>
